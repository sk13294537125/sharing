...
public class Base64Util {
	...
	public static final String encode(final byte[] src) {
		return encode(src, 0, src.length, false);
	}
	 ...
	 */
	public static final String encode(final byte[] src, final int off, final int len, final boolean breakLines) {
	 ...
	 *             if <code>src</code> contains an illegal Base-64 character.
	 */
	...
	private static final int decode4to3(final byte[] src, final int sOffset, final byte[] dest, final int dOffset) {
		if (src[sOffset + 2] == EQUALS_SIGN) { // Example: Dk==
			final int outBuff = ((DECODABET[src[sOffset]] & 0xFF) << 18) | ((DECODABET[src[sOffset + 1]] & 0xFF) << 12);
		...
		try { // Example: DkLE
			final int outBuff = ((DECODABET[src[sOffset]] & 0xFF) << 18) | ((DECODABET[src[sOffset + 1]] & 0xFF) << 12)
					| ((DECODABET[src[sOffset + 2]] & 0xFF) << 6) | ((DECODABET[src[sOffset + 3]] & 0xFF));
			dest[dOffset] = (byte) (outBuff >> 16);
			dest[dOffset + 1] = (byte) (outBuff >> 8);
